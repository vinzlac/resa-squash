...
export default function BookingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'my-bookings' | 'all-bookings'>('my-bookings');
  const [bookings, setBookings] = useState<Booking[]>([]);
  const [bookingsWithoutId, setBookingsWithoutId] = useState<MergeBooking[]>([]);
  ...
  // Fonction pour charger les bookings fusionnés
  ...
  // Charger les bookings fusionnés quand on passe sur l'onglet "all-bookings"
  useEffect(() => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Recharger les données fusionnées si on est sur le 2ème onglet
      const fetchMergedData = async () => {
        try {
          ...
          console.error('Erreur lors du rechargement des données fusionnées:', err);
        }
      };

      await fetchData();
      
      // Si on est sur le 2ème onglet, recharger aussi les données fusionnées
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center space-x-3">
                    <div className="flex-shrink-0">
                      <div className={`w-3 h-3 rounded-full ${
                        booking.bookingId ? 'bg-green-400' : 'bg-orange-400'
                      }`}></div>
                    </div>
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                        Réservation #{booking.bookingId ? booking.bookingId.slice(-8) : booking.sessionId.slice(-8)} sur le court {getCourtNumberFromClubId(booking.clubId)}
                      </h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        Session: {booking.sessionId.slice(-8)} • Prise par {getUserName(booking.bookingActionUserId)}
                        {booking.createdAt && ` le ${formatCreatedAt(booking.createdAt)}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900 dark:text-white">
                        {getUserName(booking.userId)} • {getUserName(booking.partnerId)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col space-y-2 ml-4">
                  <button
                    onClick={() => handleDelete(booking as Booking)}
                    className="inline-flex items-center px-3 py-2 border border-red-300 dark:border-red-600 shadow-sm text-sm leading-4 font-medium rounded-md text-red-700 dark:text-red-400 bg-white dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-red-900/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                  >
                    <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
