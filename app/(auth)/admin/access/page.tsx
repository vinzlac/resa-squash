...
export default function AccessPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAuthorizedUsers(authorizedData);
      } catch (err) {
  ...
  const handleToggleAccess = async (userId: string) => {
    const action = authorizedUsers.includes(userId) ? 'remove' : 'add';
  ...
  const getAuthorizedLicensees = () => licensees.filter(licensee => 
    authorizedUsers.includes(licensee.user[0]._id)
  );
                ...
                <div className="space-y-2">
                  {getAuthorizedLicensees().map(licensee => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleToggleAccess(licensee.user[0]._id)}
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            >
                              <svg
                                className="w-6 h-6"
                                fill={authorizedUsers.includes(licensee.user[0]._id) ? 'currentColor' : 'none'}
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                                <path
